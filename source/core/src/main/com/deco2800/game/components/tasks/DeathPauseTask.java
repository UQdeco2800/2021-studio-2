...
public class DeathPauseTask extends ChaseTask implements PriorityTask {
    ...
    private void playElfDead() {
        ...
        switch (numCase) {
            case 1:
                ServiceLocator.getResourceService().getAsset("sounds/death_1.mp3", Sound.class).play(0.5f);
            case 0:
                ServiceLocator.getResourceService().getAsset("sounds/death_2.mp3", Sound.class).play(0.5f);
        }
    }

    private void playElfBossDeath() {
        ServiceLocator.getResourceService().getAsset("sounds/boss_death.mp3", Sound.class).play(0.5f);
    }
